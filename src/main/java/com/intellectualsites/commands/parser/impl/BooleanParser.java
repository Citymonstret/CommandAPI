...
public class BooleanParser extends Parser<Boolean> {
    ...
    static {
        trueValues = Collections
                .unmodifiableList(Arrays.asList("yes", "true", "on", "1"));
        falseValues = Collections
                .unmodifiableList(Arrays.asList("no", "false", "off", "0"));
    }
    ...
    public ParserResult<Boolean> parse(String in) {
        if (trueValues.contains(in.toLowerCase())) {
            return new ParserResult<Boolean>(true, true);
        } else if (falseValues.contains(in.toLowerCase())) {
            return new ParserResult<Boolean>(false, true);
        }
        return new ParserResult<Boolean>(in + " is not a boolean");
    }
}
